...
<p align="center"><img src="https://user-images.githubusercontent.com/45402031/96075853-ee681e80-0ee6-11eb-9602-3bf16cfbe937.png" width="40%"></p>
...
- [**선택 정렬 소스코드 (Python)**](Source/선택정렬.py)
...
- [**삽입 정렬 소스코드 (Python)**](Source/정렬.py)
...
- [**귁 정렬 소스코드: 일반적인 방식 (Python)**](Source/퀵정렬(일반적).py)

- [**퀵 정렬 소스코드: 파이썬의 장점을 살린 방식**](Source/퀵정렬(파이썬장점).py)
...
- [**계수 정렬 소스코드 (Python)**](Source/계수정렬.py)
...
- **소수 (Prime Number)**
  -  소수란 <u>1보다 큰 자연수 중에서 1과 자기 자신을 제외한 자연수로는 나누어 떨어지지 않는 자연수</u> 입니다.
...
- **우선순위 큐**는 <u>우선순위가 가장 높은 데이터를 가장 먼저 삭제하는 자료구조</u>입니다.
...
- 트리는 가계도와 같은 **계층적인 구조**를 표현할 수 있을 때 사용할 수 있는 자료구조
- 트리 관련 용어
...
- **대표적인 트리 순회** 방법은 다음과 같습니다
  ...
  - **중위 순회(in-order traverse)**: 왼쪽 자식을 방문한 뒤 루트를 방문합니다
...
- **다익스트라 최단 경로 알고리즘**
  1. 출발 노드를 설정합니다.
  2. 최단 거리 테이블을 초기화합니다. (INF, 무한)
  3. 방문하지 않은 노드 중에서 최단 거리가 가장 짧은 노드를 선택합니다.
  4. 해당 노드를 거쳐 다른 노드로 가는 비용을 계산하여 최단 거리 테이블을 갱신합니다.
  5. 위 과정에서 3번과 4번을 반복합니다.

- 음수 간선이 포함된다고 해서 최소 비용을 구할 수 없는 것은 아니다
  - 하지만 음수 간선의 순환이 포함된다면 최소 비용을 구할 수 없다
...
- [**벨만 포드 알고리즘 소스코드(Python)**](Source/벨만포드알고리즘.py)
